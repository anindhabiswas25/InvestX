@echo off
echo ========================================
echo   InvestX Local Development Server
echo   Blockchain: Stellar (Testnet)
echo ========================================
echo.

echo [1/2] Starting Backend API (port 5000)...
start "InvestX Backend - Stellar" cmd /k "node backend/server.js"

echo [2/2] Starting Frontend React App (port 3000)...
start "InvestX Frontend - Stellar" cmd /k "cd frontend && npm start"
