...
impl InvestXEscrow {
    ...
    pub fn deposit(e: Env, business_hash: BytesN<32>, investor: Address, amount: i128) {
        ...
        e.storage().persistent().extend_ttl(&dep_key, BALANCE_LIFETIME_THRESHOLD, BALANCE_BUMP_AMOUNT);
        ...
        e.storage().persistent().extend_ttl(&biz_key, BALANCE_LIFETIME_THRESHOLD, BALANCE_BUMP_AMOUNT);

        // Update total
        let total: i128 = e.storage().instance().get(&DataKey::TotalDeposited).unwrap_or(0);
        e.storage().instance().set(&DataKey::TotalDeposited, &(total + amount));
    }
    ...
    pub fn get_business_balance(e: Env, business_hash: BytesN<32>) -> i128 {
        ...
        e.storage().persistent().get(&DataKey::BusinessBalance(business_hash)).unwrap_or(0)
    }

    /// Get total deposited across all campaigns
    pub fn get_total_deposited(e: Env) -> i128 {
        e.storage().instance().get(&DataKey::TotalDeposited).unwrap_or(0)
    }
