  ...
  if (proposalType === 0 && business) {
    ...
    if (!fullBusiness.tokenDetails?.contractAddress) {
      try {
        ...
        fullBusiness.escrowAddress = process.env.ESCROW_CONTRACT_ADDRESS || "";
    ...
    console.log(`  ✅ Business "${fullBusiness.name}" approved — now fundraising`);
  ...
  if (proposalType === 1 && business) {
    try {
      ...
      if (divRecord) {
        ...
        try {
          await dividendDistributorService.rejectAfterVote(
            business._id.toString(),
            depositIndex
          );
          console.log(`  ❌ Distribution rejected on-chain — XLM returned`);
        } catch (contractErr) {
          console.error("  Failed to reject on-chain:", contractErr.message);
        }
