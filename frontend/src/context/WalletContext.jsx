...
export const WalletProvider = ({ children }) => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("investx_stellar_address");
    if (stored) {
      ...
      if (normalized) {
        ...
        localStorage.removeItem("investx_stellar_address");
      }
    }
  }, []);
        ...
        if (publicKey) {
          ...
          if (auth?.walletLogin) {
            try {
              ...
              setIsConnected(true);
              localStorage.setItem("investx_stellar_address", publicKey);
  ...
  const completeSignup = useCallback(async (walletAddr, name, email) => {
    ...
    try {
      ...
      setIsConnected(true);
      localStorage.setItem("investx_stellar_address", walletAddr);
      setShowSignupModal(false);
      setPendingWalletAddress(null);
      toast.success("Welcome to InvestX! Your account has been created.");
    } catch (err) {
